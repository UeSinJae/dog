...
public class DogCartSearchService {
	public ArrayList<Cart> getCartSearchLsit(int startMoney,int endMoney,HttpServletRequest request){
		//request로부터 session영역을 얻어온 후
		HttpSession session=request.getSession();
		//session영역에 공유한 cartList속성값인 장바구니 목록 객체를 얻어와
		ArrayList<Cart> oldCartList = (ArrayList<Cart>)session.getAttribute("cartList");
		
		//장바구니 목로을 반목하면서 검색 범위에 해당하는 장바구니 항목을 찾아서 새로 생성한 ArrayList<Cart>객체에 추가
