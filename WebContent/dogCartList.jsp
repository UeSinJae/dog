<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"
    isELIgnored = "false"%> <!--  isELIgnored = "false" 최신이클립스는 자동으로설정됨 생략가능 -->
<!-- 페이지 지시자에 core라이브러리를 사용하겠다 하고 jstl선언해야함. c접두어로 시작 -->
...
//전체체크박스를 통해서 각 장바구니 항목의 체크박스를 체크 또는 해제하는 함수
function checkAll(theForm){
	if(theForm.remove.length == undefined){//장바구니 항목이 1개일 때 length가 undefined
...
function checkQty(kind, qty){
	if(qty != 1){//현재 수량이 1이 아니면(즉, 1보다 크면)
		//장바구니 항목 수량 감소 요청을 한다. ★이 떄, kind값이 한글이면 인코딩처리하여 한글파라미터로 전소
...
NullPointerException이 발생하지 않도록 if문으로 처리한다. -->
	...
	<form method="post">
		<table>
			<tr id="select">
				...
				<!-- 검색버튼을 클릭하면 최하가격과 최고가격으로 장바구니 항목을 검색 -->
				<input type="submit" value="검색" formaction="dogCartSearch.dog">
				</td>				
			</tr>
			<!-- 가격별 검색부분 처리 끝 -->
			<tr class = "tr_top">
				<td> <!-- 전체 체크박스:체크하면 모든 장바구니 항목의 체크박스가 체크,
			...
			<tr>
				...
				<td>${status.index+1 }</td> <!-- 번호값 : status.index는 0부터시작 -->
				<td><img src="images/${cart.image}" id="cartImage"></td>
				<td>${cart.kind}</td>
				<td>${cart.price }</td>
				<td>
				  <!-- ▲클릭시 : 장바구니 항목의 수량 증가 요청(이 때, utf-8로 인코딩 처리된 kind값을 파라미터로 전송해야함) -->
					<a href="dogCartQtyUp.dog?kind=${cart.encodingKind}"><img src = "images/up.jpg" id="upImage" border=0></a>
					<br>
					${cart.qty }<!-- 현재 수량 -->
					<br>
				 <!-- ▼클릭시 : 장바구니 항목의 수량 감소 요청하기 위해 함수 호출 checkQty()(이 때, kind값과 현재 수량을 매개값으로 전송해야 함) -->
					<a href="javascript:checkQty('${cart.kind }','${cart.qty }')"><img src = "images/down.jpg" id="downImage" border=0></a>
				</td>
			</tr>
			</c:forEach>
			<tr>
				<td colspan="6">총금액 : ${totalMoney}원</td>
			</tr>
			<tr>
